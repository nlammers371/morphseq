...
master_cds <- combine_cds(list(hot_cds_28, ref1_cds, ref2_cds), 
...
# Assuming the cell metadata is available as a data frame:
# metadata <- pData(ccs)  # or colData(ccs) if appropriate

# # Identify indices for each protocol group:
# group1_idx <- which(metadata$dis_protocol == 1)
# group2_idx <- which(metadata$dis_protocol == 2)

# # Determine the minimum number of cells between the two groups:
# n_keep <- min(length(group1_idx), length(group2_idx))

# # Randomly sample n_keep indices from each group:
# set.seed(123)  # for reproducibility
# sampled_group1 <- sample(group1_idx, n_keep)
# sampled_group2 <- sample(group2_idx, n_keep)

# # Combine the indices:
# selected_idx <- c(sampled_group1, sampled_group2)

# # Subset the ccs object (assuming cells are columns):
# ccs <- ccs[, selected_idx]
...
mdl_name_list <- c("t_spline_lin4", "t_spline_inter_6knots")
formula_list <- c(paste0(time_formula, " + dis_protocol + expt"), paste0(time_formula, " * dis_protocol + expt"))
