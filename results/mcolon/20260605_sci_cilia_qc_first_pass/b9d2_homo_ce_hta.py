"""
b9d2 homozygous-conditional phenotype transfer.

Train only on b9d2 homozygous-mutant reference embryos with phenotypes CE vs HTA. BA_rescue is merged into HTA; wildtype is excluded.

This answers the conditional question:
    given a b9d2 homozygous mutant, is the phenotype CE or HTA?

Run:
    conda run -n segmentation_grounded_sam --no-capture-output python \
        results/mcolon/20260605_sci_cilia_qc_first_pass/b9d2_homo_ce_hta.py
"""
...
OUT = RUN_DIR / "plots" / "b9d2" / "b9d2_homo_only_and_its_phenotypes"
SEQ_OUT = RUN_DIR / "plots" / "sequenced_focus" / "b9d2" / "homozygous_focus"
TR = RUN_DIR / "transfer_results"

PHENO_ORDER = HOMO_PHENOTYPE_ORDER["b9d2"]
DATASET_LABEL = "b9d2"
STAGE_GRID = STAGE_GRIDS["b9d2"]
FILE_PREFIX = "b9d2_homo_ce_hta"

PHENO_COLORS = PHENOTYPE_COLORS["b9d2"]
...
def _b9d2_inputs() -> tuple[pd.DataFrame, pd.DataFrame, list[str], list[str], dict[str, float]]:
    cfg = T.DATASETS["b9d2"]
    ...
    ref = T._load(cfg["ref"], feat, gene_hint="b9d2")

    qparts = []
    start_age = {}
    for exp, path in zip(queries, qpaths):
        q = T._load(path, feat, gene_hint="b9d2")
...
def _homo_directional_reference(ref: pd.DataFrame) -> pd.DataFrame:
    ...
    r.loc[r[T.PHENO_COL] == "BA_rescue", T.PHENO_COL] = "HTA"
...
def _target_cv_predictions(
    ...
    targets = tuple(TARGET_HPF_WINDOWS["b9d2"] if targets is None else targets)
    ...
    for target in targets:
        ...
        if emb[T.PHENO_COL].nunique() < 2:
            continue

        if emb["experiment_id"].nunique() >= 2:
            splits = []
            for exp in sorted(emb["experiment_id"].dropna().unique()):
                train_idx = emb.index[emb["experiment_id"] != exp].to_numpy()
                test_idx = emb.index[emb["experiment_id"] == exp].to_numpy()
                splits.append((train_idx, test_idx, "leave-one-experiment-out"))
        else:
            # Early b9d2 homo-only windows have CE/HTA labels but only one reference experiment.
            # Fall back to stratified k-fold so the panel reflects class separability instead of
            # being silently omitted.
            counts = emb[T.PHENO_COL].value_counts()
            n_splits = int(min(5, counts.min()))
            if n_splits < 2:
                continue
            cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=0)
            splits = [(train_idx, test_idx, f"{n_splits}-fold stratified")
                      for train_idx, test_idx in cv.split(emb[feat], emb[T.PHENO_COL])]

        for train_idx, test_idx, cv_strategy in splits:
            train = emb.loc[train_idx]
            test = emb.loc[test_idx]
...
def _plot_target_confusion(cv: pd.DataFrame) -> None:
    ...
    for ax, target in zip(axes[0], targets):
        ...
        if sub.empty:
            ax.imshow(np.full((len(PHENO_ORDER), len(PHENO_ORDER)), np.nan), cmap="Blues", vmin=0, vmax=1)
            ax.text(0.5, 0.5, "no valid CV", ha="center", va="center", fontsize=9, transform=ax.transAxes)
    ...
    fig.suptitle("b9d2 homo-only CE/HTA phenotype CV at target hpf windows", fontsize=11)
    plt.tight_layout(rect=[0, 0, 1, 0.92])
    _save(fig, OUT / "b9d2_homo_ce_hta_reference_confusion_target_hpf_pm2.png")
...
def _short_plate(plate: str) -> str:
    return str(plate).replace("_b9d2", "").replace("b9d2_", "")
...
def _plot_minibars(pred: pd.DataFrame, *, homo_only: bool) -> None:
    p = pred.dropna(subset=["stage"]).copy()
    if homo_only:
        p = p[p["truth_group"] == "b9d2_homozygous -> homozygous"].copy()
        groups = ["b9d2_homozygous -> homozygous"]
        suffix = "homo_only"
        title = "b9d2 homo-only model: sequenced homozygous predicted phenotype counts"
    else:
        groups = _truth_group_order()
        suffix = "all_true_genotypes"
        title = "b9d2 homo-only model: sequenced predicted phenotype counts by true genotype"
    if p.empty:
        return
    p["stage"] = p["stage"].astype(float).round().astype(int)
    stages = STAGE_GRID
    ...
    _save(fig, SEQ_OUT / f"b9d2_homo_ce_hta_predicted_phenotype_minibars_{suffix}.png")
...
def main() -> None:
    ...
    ref_raw, qry, feat, queries, start_age = _b9d2_inputs()
    ref = _homo_directional_reference(ref_raw)
    seqlk = S.build_sequenced_lookup(queries)

    print("b9d2 homo-only CE/HTA phenotype transfer")
    ...
    emb.to_csv(TR / "b9d2_homo_ce_hta_predictions.csv", index=False)

    seq = emb[emb["sequenced"] > 0].copy()
    seq.to_csv(TR / "b9d2_homo_ce_hta_sequenced_predictions.csv", index=False)
    ...
    cv.to_csv(TR / "b9d2_homo_ce_hta_reference_cv_predictions_target_hpf_pm2.csv", index=False)
    _plot_target_confusion(cv)

    homo = seq[seq["truth_group"] == "b9d2_homozygous -> homozygous"]
    if not homo.empty:
