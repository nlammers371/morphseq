"""
viz.phase0 — Phase 0 visualization functions.

Moved from p0_viz.py. All matplotlib calls go through viz.contract wrappers;
...
def plot_displacement_suite(
    ...
    for idx, (label, (u_map, v_map)) in enumerate(mean_disp.items()):
        ...
        # Background: embryo mask — use extent to align with pixel-center quiver coords
        contract.imshow(ax, mask_ref.astype(float), cmap="gray", alpha=0.3,
        ...
        contract.embryo_outline(mask_ref, ax, color="white")
    ...
    for idx, (label, (u_map, v_map)) in enumerate(mean_disp.items()):
        ...
        im = contract.imshow(ax, display, cmap="viridis", interpolation="bilinear")
        contract.embryo_outline(mask_ref, ax)
...
def plot_s_map(
    ...
    im = contract.imshow(ax, display, cmap="viridis", vmin=0, vmax=1, interpolation="bilinear")
    contract.embryo_outline(mask_ref, ax, color="white")
