"""
viz.qc — Phase 0 QC visualization functions.

Moved from p0_qc.py. All matplotlib calls go through viz.contract wrappers;
direct ax.imshow / ax.contour / plt.imshow / plt.contour are banned here.

See viz.contract for the coordinate-frame specification.
"""
...
try:
    ...
    _HAS_OUTLIER_DETECTION = True
except ImportError:
    _HAS_OUTLIER_DETECTION = False
...
def compute_iqr_outliers(
    ...
    if _HAS_OUTLIER_DETECTION:
        ...
        outlier_flag = (total_cost_C < lower_bound) | (total_cost_C > upper_bound)
...
def plot_qc2_worst_samples(
    ...
    for idx, rank in enumerate(range(top_n)):
        ...
        if target_masks_canonical is not None:
            ...
            contract.imshow(ax, target_overlay, cmap="gray", vmin=0.0, vmax=1.0,
            ...
            contract.imshow(ax, target_only, cmap="Blues", vmin=0.0, vmax=1.0,
                            alpha=0.45, interpolation="nearest")
            contract.imshow(ax, ref_only, cmap="Purples", vmin=0.0, vmax=1.0,
                            alpha=0.35, interpolation="nearest")

        im = contract.imshow(ax, display, cmap="hot", alpha=0.90,
                             interpolation="nearest", vmin=vmin_shared, vmax=vmax_shared)

        contract.contour(ax, mask_ref.astype(float), levels=[0.5],
                         colors=["white"], linewidths=0.5, alpha=0.8)
        if target_masks_canonical is not None:
            contract.contour(ax, target_mask.astype(float), levels=[0.5],
                             colors=["#9ecae1"], linewidths=0.9, alpha=0.95)
...
def plot_qc4_outlier_diagnostics(
    ...
    for rank, i in enumerate(outlier_idx_sorted, start=1):
        ...
        ax = axes[0]
        contract.imshow(ax, np.where(ref_mask_bool, 1.0, np.nan),
                        cmap="gray", alpha=0.35, interpolation="nearest")
        contract.imshow(ax, np.where(target_mask, 1.0, np.nan),
                        cmap="Blues", alpha=0.35, interpolation="nearest")
        target_only = np.where(target_mask & (~ref_mask_bool), 1.0, np.nan)
        ref_only = np.where(ref_mask_bool & (~target_mask), 1.0, np.nan)
        contract.imshow(ax, target_only, cmap="Blues", alpha=0.6, interpolation="nearest")
        contract.imshow(ax, ref_only, cmap="Purples", alpha=0.45, interpolation="nearest")
        contract.contour(ax, ref_mask_bool.astype(float), levels=[0.5],
        ...
        im = contract.imshow(ax, cost_display, cmap="hot", interpolation="nearest")
        contract.contour(ax, ref_mask_bool.astype(float), levels=[0.5],
                         colors=["white"], linewidths=0.8, alpha=0.9)
        contract.contour(ax, target_mask.astype(float), levels=[0.5],
                         colors=["#9ecae1"], linewidths=0.9, alpha=0.95)
