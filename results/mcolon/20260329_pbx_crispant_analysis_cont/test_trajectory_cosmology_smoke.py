...
def _make_toy_data(n_embryo=3, n_time=4, n_feat=2, seed=0):
    """Small fully-observed toy CosmologyData for unit tests."""
...
def _toy_positions(n_embryo=2, n_time=3):
    """Simple 2D positions for math tests."""
    pos = np.zeros((n_embryo, n_time, 2))
    mask = np.ones((n_embryo, n_time), dtype=bool)
    return pos, mask


# ---------------------------------------------------------------------------
# Group 0: Existing real-data tests (regression)
# ---------------------------------------------------------------------------
...
def test_condensation_smoke_real_pbx_input_is_finite():
    ...
    result = run_condensation(
        x0=x0, mask=data.mask,
        config=CondensationConfig(max_iter=5),
...
# ---------------------------------------------------------------------------
# Group A: schema validate() boundary tests
# ---------------------------------------------------------------------------

def test_validate_rejects_unsorted_times():
    data = _make_toy_data()
    # flip time_values order — breaks sorted invariant
...
def test_validate_rejects_non_nan_unobserved():
    data = _make_toy_data()
    # carve out one observed entry and then set its feature to 0 while marking unobserved
    data.mask[0, 0] = False
    data.features[0, 0, :] = 0.0   # should be NaN when unobserved
    with pytest.raises(AssertionError, match="NaN"):
        validate(data)


def test_validate_rejects_inconsistent_labels():
    """_build_canonical should raise if one embryo has two different label values."""
    # Build a minimal CSV where embryo e00 has two genotypes
    df = pd.DataFrame({
        "embryo_id": ["e00", "e00", "e01", "e01"],
        "time_bin_center": [1.0, 2.0, 1.0, 2.0],
        "genotype": ["wt", "mut", "wt", "wt"],   # e00 is inconsistent
...
# ---------------------------------------------------------------------------
# Group B: math unit tests (toy data)
# ---------------------------------------------------------------------------

def test_coherence_delta0_equals_kernel():
    """With delta=0, C_ij(t) == K_s(x_i(t), x_j(t)) for all observed pairs."""
    n_embryo, n_time = 3, 4
    rng = np.random.default_rng(1)
    pos = rng.standard_normal((n_embryo, n_time, 2))
    mask = np.ones((n_embryo, n_time), dtype=bool)
    sigma = 0.5
    ...
    for t in range(n_time):
        ...
        np.testing.assert_allclose(C[:, :, t], K, atol=1e-10,
                                   err_msg=f"delta=0 coherence != kernel at t={t}")


def test_coherence_identical_trajectories():
    """Two embryos always at the same position → C ≈ 1.0."""
    pos = np.zeros((2, 5, 2))   # both at origin
    mask = np.ones((2, 5), dtype=bool)
    C = compute_coherence(pos, mask, sigma=0.5, delta=3)
    # kernel of identical points = exp(0) = 1
...
def test_coherence_far_trajectories():
    """Two embryos 100 units apart → C ≈ 0.0."""
    pos = np.zeros((2, 5, 2))
    pos[1, :, 0] = 100.0   # embryo 1 far away on x-axis
    mask = np.ones((2, 5), dtype=bool)
    C = compute_coherence(pos, mask, sigma=0.5, delta=3)
    assert C[0, 1, :].max() < 1e-10
    assert C[1, 0, :].max() < 1e-10


def test_attraction_pulls_together():
    """Gradient descent on attraction energy moves embryos toward each other.

    Convention: grad = dE/dx. Update is x -= lr * grad.
    E_attract = -K_s * C, so dE/dx_i = +W*(x_i - x_j)/sigma^2.
    This points AWAY from x_j, so subtracting it moves x_i toward x_j.
    """
    pos = np.zeros((2, 1, 2))
    pos[0, 0, :] = [-1.0, 0.0]
    pos[1, 0, :] = [+1.0, 0.0]
    ...
    # grad[0] points in -x direction (away from j); subtracting moves i toward j
    assert grad[0, 0, 0] < 0, "dE/dx_i should point away from j (descent moves toward j)"
    assert grad[1, 0, 0] > 0, "dE/dx_j should point away from i (descent moves toward i)"

    # Verify gradient descent step moves embryos closer
    step = pos - 0.1 * grad
    dist_before = abs(pos[0, 0, 0] - pos[1, 0, 0])
    dist_after = abs(step[0, 0, 0] - step[1, 0, 0])
    assert dist_after < dist_before, "one gradient step should reduce distance"


def test_repulsion_pushes_apart():
    """Gradient descent on repulsion energy moves embryos away from each other.

    Convention: grad = dE/dx. Update is x -= lr * grad.
    E_repel = epsilon_r / (dist^2 + eta), so dE/dx_i points toward x_j.
    Subtracting it moves x_i away from x_j.
    """
    pos = np.zeros((2, 1, 2))
    pos[0, 0, :] = [-0.5, 0.0]
    pos[1, 0, :] = [+0.5, 0.0]
    ...
    # grad[0] points in +x direction (toward j); subtracting moves i away from j
    assert grad[0, 0, 0] > 0, "dE/dx_i should point toward j (descent moves away from j)"
    assert grad[1, 0, 0] < 0, "dE/dx_j should point toward i (descent moves away from i)"

    # Verify gradient descent step increases distance
    step = pos - 0.1 * grad
    dist_before = abs(pos[0, 0, 0] - pos[1, 0, 0])
    dist_after = abs(step[0, 0, 0] - step[1, 0, 0])
    assert dist_after > dist_before, "one gradient step should increase distance"


def test_elasticity_straight_line_zero_bend():
    """Collinear, equidistant trajectory → bend energy = 0."""
    # embryo traces a straight line: (0,0), (1,0), (2,0), (3,0)
    pos = np.zeros((1, 4, 2))
    pos[0, :, 0] = [0.0, 1.0, 2.0, 3.0]
    mask = np.ones((1, 4), dtype=bool)

    energy, grad = elasticity(pos, mask, lambda_stretch=1.0, lambda_bend=1.0)

    # Bend term: x(t+1) - 2x(t) + x(t-1) = 1 - 2 + 1 = 0 for equidistant straight line
    # Compute stretch contribution separately to isolate bend
    stretch_energy = 1.0 * 3 * (1.0 ** 2)   # 3 steps of length 1, lambda=1
    assert abs(energy - stretch_energy) < 1e-10, (
        f"bend energy should be 0 for straight line, got {energy - stretch_energy:.2e}"
    )


def test_finite_difference_gradient():
    """Analytic gradient matches numerical finite differences on a tiny toy problem."""
    rng = np.random.default_rng(42)
    n_embryo, n_time = 2, 3
    pos = rng.standard_normal((n_embryo, n_time, 2)) * 0.3
    x0 = pos.copy()
    mask = np.ones((n_embryo, n_time), dtype=bool)
    C = compute_coherence(pos, mask, sigma=0.5, delta=1)

    kwargs = dict(
        x0=x0, mask=mask, coherence=C,
        sigma=0.5, epsilon_r=0.01, eta=1e-4,
        lambda_stretch=0.1, lambda_bend=0.05, mu=0.5,
    )
    ...
    np.testing.assert_allclose(
        grad_analytic, grad_numerical, atol=1e-4, rtol=1e-3,
        err_msg="Analytic gradient does not match finite differences",
    )


# ---------------------------------------------------------------------------
# Group C: stopping scale tests
# ---------------------------------------------------------------------------
...
def test_displacement_metrics_proportional_to_shift():
    """Shifting all points by a constant → disp_max_rel == shift / scale."""
    x = np.zeros((3, 3, 2))
    mask = np.ones((3, 3), dtype=bool)
    # put points at known positions so scale is predictable
    x[0, :, 0] = 1.0
    x[1, :, 0] = -1.0
    scale = reference_scale_from_positions(x, mask)
