...
### API changes
- `mu0` → `fidelity_init_strength`: initial anchor pull magnitude
- `gamma` → `fidelity_half_life`: per-iteration retention multiplier
  (μ_n = fidelity_init_strength × fidelity_half_life^n)
  - 0.99 = slow decay, just enough to matter (onset regime on Y-benchmark)
  - 1.0  = no decay — permanent anchor, different dynamical regime entirely
  - 0.1  = fast decay, anchor gone in ~10 iterations
- New defaults: `fidelity_init_strength=1.0`, `fidelity_half_life=0.99`
- Elasticity defaults updated to weak-on: `stretch_strength_mult=0.001`, `bend_strength_mult=0.001`
  (confirmed inert at these values on Y-benchmark)

### fidelity_half_life sweep result
Swept `fidelity_half_life` from 0.01 → 0.9999 with `fidelity_init_strength=1.0` fixed.
`branch_sep_late` and `trunk_linearity_early` both flat (identical to isotropic) for half_life < 0.99.
Effect first appears at half_life=0.99, grows slightly to 0.9999. Trunk linearity completely
unaffected across full range. Fidelity only shifts branch separation, not trunk shape.
...
value (log scale). Vertical dashed line = default value. Thick dotted line = isotropic baseline.

Families: repulsion_strength_mult | fidelity_init_strength | fidelity_half_life
          epsilon_void             | stretch_strength_mult  | bend_strength_mult

Generated by `bifurcating_trunk_sandbox.py --fidelity-half-life-sweep` on the Y-benchmark
(n=40/branch, 300 iter). Separate y-axes per subplot because each force has a different
dynamic range on each metric.
