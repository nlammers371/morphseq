...
## Phase C: API rename + per-family sweep figure (2026-03-31)

### API changes
- `mu0` → `fidelity_init_strength`: initial anchor pull magnitude (0.0 = off)
- `gamma` → `fidelity_half_life_iters`: number of **solver iterations** for fidelity weight to halve
  - Unit: **solver iterations** — not time bins, not hpf, not trajectory steps
  - Conversion: γ = 2^(−1/h) where h = fidelity_half_life_iters
  - Helper: `gamma_from_half_life_iters(h)` in `temporal_sandbox.py`
  - None = no decay (constant anchor — different dynamical regime)
  - 5   ≈ fast decay (anchor gone in ~15 iters)
  - 70  ≈ old gamma=0.99 onset regime
  - 200 = persistent anchor across a 300-iter run
- Defaults: `fidelity_init_strength=0.0` (off), `fidelity_half_life_iters=70.0` (metadata only when off)
- Elasticity defaults: `stretch_strength_mult=0.001`, `bend_strength_mult=0.001` (confirmed inert)

### fidelity_half_life sweep result (in retention units, now superseded by iter-units sweep)
Swept old `fidelity_half_life` (γ) from 0.01 → 0.9999. Effect flat below γ=0.99 (≈ h=69 iters).
Onset at γ=0.99, slight growth to 0.9999. Trunk linearity unaffected across full range.
Fidelity only shifts branch separation.

### Figure: force family sweeps (2×3 grid)
**File**: `results/mcolon/20260329_pbx_crispant_analysis_cont/results/force_calibration_v1/force_sweeps/force_sweeps.png`

2×3 grid, one facet per force family. Each facet: two stacked subplots sharing x-axis —
top = `branch_sep_late` (blue), bottom = `trunk_linearity_early` (red). X-axis = parameter
value (log scale). Vertical dashed line = default value. Thick dotted isotropic baseline.
Green shaded region = inert zone (below no_change threshold). Green dash-dot line = no_change threshold.

Families: repulsion_strength_mult | fidelity_strength_mult | fidelity_half_life_iters
          epsilon_void             | stretch_strength_mult  | bend_strength_mult

Generated by `bifurcating_trunk_sandbox.py --fidelity-half-life-sweep`
(n=30/branch, n_iter=500, lr=1e-4 — calibrated solver tempo).
Separate y-axes per subplot — each force has a different dynamic range on each metric.

**Updated defaults** (all just below no_change on well-initialized benchmark):
- repulsion_strength_mult=0.005 (no_change≤0.0071)
- fidelity_strength_mult=0.25 (no_change≤0.334), fidelity_half_life_iters=70
- epsilon_void=0.014 (no_change≤0.018)
- stretch_strength_mult=0.04, bend_strength_mult=0.04 (no_change≤0.049 each)

See also: `results/force_calibration_v1/FORCE_CALIBRATION_SUMMARY.md`
and `results/force_calibration_v1/solver_tempo_selection.png` for lr/n_iter rationale.

---

## Phase D: Solver-tempo calibration (next step as of 2026-04-01)

### Motivation

A force that appears inert may be inert for two distinct reasons:
1. Its coefficient is truly too small to matter
2. The solver stops before cumulative effect has time to accumulate

The current sweeps cannot distinguish these. The next calibration step is to find a solver
regime where weak / moderate forces have enough optimization bandwidth to interact, so that
"inert" truly means inert — not merely under-integrated.

### Goal

Find a (learning_rate, n_iter) pair such that:
- **Stable**: no explosion, oscillation, or spread inflation
- **Bandwidth**: small forces have enough iterations to express themselves before judgment
- **Economical**: cheap enough for routine use

This is not about finding the fastest solver. It is about finding the slow, stable regime
where force comparisons are trustworthy.

### Planned sweep

Grid: `lr ∈ [1e-5, 3e-5, 1e-4, 3e-4, 1e-3]` × `n_iter ∈ [50, 100, 200, 300, 500, 800]`

Conditions per cell:
- A: isotropic baseline
- B: weak fidelity (fidelity_init_strength=1.0, fidelity_half_life_iters=70)
- C: weak elasticity (stretch_strength_mult=0.001)

For each cell, track per-iteration curves of: branch_sep_late, trunk_linearity_early,
within_branch_spread_ratio. Ask:
- Does the baseline stay stable?
- Do weak forces become distinguishable from baseline at longer budgets?
- When do curves flatten (convergence in metric space, not just displacement)?

### Expected outcome

A recommended default `(lr, n_iter)` pair that gives forces enough time to interact,
and a clear statement of when metrics plateau so the budget is justified.
