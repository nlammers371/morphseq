...
def main() -> None:
    ...
    # -------------------------------------------------------------------------
    # Plot AUROC heatmaps (all crispants vs wik-ab) via the faceted heatmap API
    if run_mode in {"all", "vs_wik_ab"} and not vs_wik_ab_summary.empty:
        import importlib.util

        helper_path = Path(__file__).resolve().parent / "data_viz" / "01_plot_classification_heatmaps.py"
        ...
        heatmap_path = module.save_mode_heatmaps(
            classification_dir=classification_dir,
            output_path=classification_fig_dir / f"{EXPERIMENT_LABEL}_all_crispants_vs_wik_ab_heatmaps_v2.png",
            mode_stem="all_crispants_vs_wik_ab",
    ...
    if not vs_wik_ab_summary.empty:
        ...
        if run_mode in {'all', 'vs_wik_ab'}:
            print(f"- {classification_fig_dir / f'{EXPERIMENT_LABEL}_all_crispants_vs_wik_ab_heatmaps_v2.png'}")
    if not vs_inj_ctrl_summary.empty:
