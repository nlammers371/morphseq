#!/usr/bin/env python3
"""
Predict curvature from embeddings using Gradient Boosting: Goal 1C Analysis

Same as predict_curvature_from_embeddings.py but uses Gradient Boosting
instead of Ridge regression to see if non-linear model performs better.

Key Question:
    Does Gradient Boosting predict curvature better than Ridge regression?
"""
...
FIGURE_DIR = RESULTS_DIR / 'outputs' / 'figures_gb'
TABLE_DIR = RESULTS_DIR / 'outputs' / 'tables_gb'
...
# Models to compare - Gradient Boosting
MODELS_TO_TEST = {
...
def main():
    """Run curvature prediction analysis with Gradient Boosting."""
    print("\n" + "="*80)
    print("PREDICTING CURVATURE FROM EMBEDDINGS: GRADIENT BOOSTING")
    print("="*80)
    ...
    # Save results table
    results_file = TABLE_DIR / 'model_results_gb.csv'
    ...
    for metric, metric_results in results_by_metric.items():
        ...
        fig = plot_predictions_vs_actual(
            pred_df,
            title=f'{metric} (Gradient Boosting)',
        ...
        if len(importance_df) > 0:
            ...
            ax.set_xlabel('Feature Importance (GB)', fontsize=12)
            ax.set_ylabel('Count', fontsize=12)
            ax.set_title(f'Distribution of Feature Importances (Gradient Boosting)\n{metric}', fontsize=14)
    ...
    print(f"\n✓ Overall Results (Gradient Boosting):")
