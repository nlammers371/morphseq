#!/usr/bin/env python3
"""
Build Total Length reference curves for wildtype embryos with GAUSSIAN SMOOTHING on trajectories.

Applies Gaussian smoothing (sigma=1.5) to individual trajectories BEFORE computing percentiles.
This smooths out measurement noise in the raw data, resulting in cleaner reference curves.
...
Usage:
    python build_length_reference_smoothed.py
"""
...
print("(With Gaussian smoothing applied to individual trajectories)")
print("=" * 80)
...
# Apply Gaussian smoothing to individual trajectories
print(f"\nApplying {SMOOTH_METHOD} smoothing (sigma={SMOOTH_SIGMA}) to individual trajectories...")
if 'embryo_id' in df_ref.columns:
    # Smooth each embryo's trajectory independently
    for embryo_id in df_ref['embryo_id'].unique():
        embryo_mask = df_ref['embryo_id'] == embryo_id
        embryo_data = df_ref[embryo_mask].sort_values('predicted_stage_hpf')

        if len(embryo_data) > 1:
            smoothed_values = gaussian_filter1d(embryo_data['total_length_um'].values, sigma=SMOOTH_SIGMA)
            df_ref.loc[embryo_mask, 'total_length_um'] = smoothed_values
    print(f"  ✓ Smoothed {df_ref['embryo_id'].nunique()} embryo trajectories")
else:
    print("  ⚠️  'embryo_id' column not found, skipping trajectory smoothing")
...
output_csv = OUTPUT_DIR / "length_reference_curves_smoothed.csv"
...
output_plot = OUTPUT_DIR / "length_reference_plot_smoothed.png"
...
# Plot individual wildtype trajectories (faded) - NOW SMOOTHED
if 'embryo_id' in df_ref.columns:
    ...
    for embryo_id in embryo_ids:
        ...
        if len(embryo_data) > 1:
            ax.plot(embryo_data['predicted_stage_hpf'], embryo_data['total_length_um'],
                   alpha=0.1, linewidth=0.5, color='gray', label='Smoothed embryo trajectories' if embryo_id == embryo_ids[0] else '')
...
ax.set_xlabel('Predicted Stage (hpf)', fontsize=12)
ax.set_ylabel('Total Length (µm)', fontsize=12)
ax.set_title('Wildtype Total Length Trajectories (Gaussian Smoothed) with Reference Bounds', fontsize=14, fontweight='bold')
ax.legend(fontsize=10)
ax.grid(alpha=0.3)

output_traj_plot = OUTPUT_DIR / "length_reference_with_trajectories_smoothed.png"
