#!/usr/bin/env python3
"""
Build Total Length reference curves for wildtype embryos using 3RD-ORDER POLYNOMIAL FITTING.

Fits cubic (degree=3) polynomials to binned percentile data for simple, interpretable reference curves.
This approach is more rigid but computationally lightweight and produces smooth curves.
...
Usage:
    python build_length_reference_polynomial.py
"""
...
print("(With 3rd-order polynomial fitting)")
print("=" * 80)
...
# Fit polynomials to percentile curves
print(f"\nFitting {POLY_DEGREE}rd-order polynomials to percentile curves...")

try:
    # Fit polynomials
    poly_p5 = np.polyfit(hpf_centers, p5_array, POLY_DEGREE)
    poly_p50 = np.polyfit(hpf_centers, p50_array, POLY_DEGREE)
    poly_p95 = np.polyfit(hpf_centers, p95_array, POLY_DEGREE)

    # Evaluate polynomials at original bin centers
    p5_poly = np.polyval(poly_p5, hpf_centers)
    p50_poly = np.polyval(poly_p50, hpf_centers)
    p95_poly = np.polyval(poly_p95, hpf_centers)

    print("  ✓ Polynomial fitting successful")
    print(f"  p5 coefficients: {poly_p5}")
    print(f"  p50 coefficients: {poly_p50}")
    print(f"  p95 coefficients: {poly_p95}")
except Exception as e:
...
output_csv = OUTPUT_DIR / "length_reference_curves_polynomial.csv"
...
ax = axes[0]
ax.plot(hpf_centers, p50_poly, 'b-', linewidth=2, label='p50 (median)')
ax.fill_between(hpf_centers, p5_poly, p95_poly, alpha=0.3, color='blue', label='p5-p95 range')
ax.plot(hpf_centers, p5_poly, 'b--', linewidth=1, alpha=0.5)
ax.plot(hpf_centers, p95_poly, 'b--', linewidth=1, alpha=0.5)
ax.set_xlabel('Predicted Stage (hpf)', fontsize=12)
ax.set_ylabel('Total Length (µm)', fontsize=12)
ax.set_title('Total Length Reference Curves from Wildtype Controls (3rd-Order Polynomial Fit)', fontsize=14, fontweight='bold')
ax.legend()
ax.grid(alpha=0.3)
...
output_plot = OUTPUT_DIR / "length_reference_plot_polynomial.png"
...
ax.fill_between(hpf_centers, p5_poly, p95_poly, alpha=0.3, color='blue', label='p5-p95 range (reference)')
ax.plot(hpf_centers, p50_poly, 'b-', linewidth=2.5, label='p50 (median)')
ax.plot(hpf_centers, p5_poly, 'b--', linewidth=1.5, alpha=0.7, label='p5')
ax.plot(hpf_centers, p95_poly, 'b--', linewidth=1.5, alpha=0.7, label='p95')

ax.set_xlabel('Predicted Stage (hpf)', fontsize=12)
ax.set_ylabel('Total Length (µm)', fontsize=12)
ax.set_title('Wildtype Total Length Trajectories with Polynomial-Fitted Reference Bounds', fontsize=14, fontweight='bold')
ax.legend(fontsize=10)
ax.grid(alpha=0.3)

output_traj_plot = OUTPUT_DIR / "length_reference_with_trajectories_polynomial.png"
...
print(f"\nReference curves (polynomial-fitted):")
print(f"  p5 range: {np.nanmin(p5_poly):.1f} - {np.nanmax(p5_poly):.1f} µm")
print(f"  p50 range: {np.nanmin(p50_poly):.1f} - {np.nanmax(p50_poly):.1f} µm")
print(f"  p95 range: {np.nanmin(p95_poly):.1f} - {np.nanmax(p95_poly):.1f} µm")
