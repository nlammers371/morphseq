#!/usr/bin/env python3
"""
Build Total Length reference curves for wildtype embryos using SPLINE FITTING.

Fits UnivariateSpline to binned percentile data for cleaner, more flexible reference curves.
This approach avoids oscillations and captures growth patterns better than low-order polynomials.

Adapted from build_length_reference_smoothed.py

Experiments: 20250912, 20251106-20251119
Filters: wildtype genotype, no chem perturbation, wik/ab genetic background

Usage:
    python build_length_reference_spline.py
"""
...
# Output
OUTPUT_DIR = Path("/net/trapnell/vol1/home/mdcolon/proj/morphseq/results/mcolon/20251204_initial_b9d2_pair_analysis/build_length_reference")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Spline configuration
SPLINE_SMOOTHING = 1e5  # Smoothing parameter - adjust for more/less smoothing

print("=" * 80)
print("BUILDING TOTAL LENGTH REFERENCE CURVES FROM WILDTYPE CONTROLS")
print("(With UnivariateSpline fitting)")
print("=" * 80)
...
has_surface_area = 'surface_area_um' in df_ref.columns
if has_surface_area:
    print("✓ surface_area_um column found - will generate reference curves for both metrics")
else:
    print("⚠️  surface_area_um column not found - will only generate total_length_um curves")

# Drop NaN values
print(f"\nDropping NaN values...")
initial_len = len(df_ref)
required_cols = ['predicted_stage_hpf', 'total_length_um']
if has_surface_area:
    required_cols.append('surface_area_um')
df_ref = df_ref.dropna(subset=required_cols)
...
# Initialize arrays for total_length_um
p5_length = np.full(len(hpf_centers), np.nan)
p50_length = np.full(len(hpf_centers), np.nan)
p95_length = np.full(len(hpf_centers), np.nan)

# Initialize arrays for surface_area_um (if available)
if has_surface_area:
    p5_area = np.full(len(hpf_centers), np.nan)
    p50_area = np.full(len(hpf_centers), np.nan)
    p95_area = np.full(len(hpf_centers), np.nan)
...
for i, hpf_center in enumerate(hpf_centers):
    ...
    if n >= 5:  # Minimum sample size for stable percentiles
        ...
        p5_length[i] = length_vals.quantile(0.05)
        p50_length[i] = length_vals.quantile(0.50)
        p95_length[i] = length_vals.quantile(0.95)

        if has_surface_area:
            area_vals = df_ref.loc[bin_mask, 'surface_area_um']
            p5_area[i] = area_vals.quantile(0.05)
            p50_area[i] = area_vals.quantile(0.50)
            p95_area[i] = area_vals.quantile(0.95)

# Summary
n_valid_bins = (~np.isnan(p50_length)).sum()
print(f"\n✓ Valid bins: {n_valid_bins} / {len(hpf_centers)} ({100*n_valid_bins/len(hpf_centers):.1f}%)")
if n_valid_bins > 0:
    print(f"  Stage range: {hpf_centers[~np.isnan(p50_length)].min():.1f} - {hpf_centers[~np.isnan(p50_length)].max():.1f} hpf")

# Fill gaps with interpolation, then edges with extrapolation
print("\nFilling gaps and edge bins...")
valid_mask = ~np.isnan(p50_length)
if valid_mask.sum() > 0:
    ...
    # Interpolate missing values in the middle (linear) - total_length_um
    p5_length = np.interp(hpf_centers, valid_hpf, p5_length[valid_indices])
    p50_length = np.interp(hpf_centers, valid_hpf, p50_length[valid_indices])
    p95_length = np.interp(hpf_centers, valid_hpf, p95_length[valid_indices])

    # Interpolate for surface_area_um if available
    if has_surface_area:
        p5_area = np.interp(hpf_centers, valid_hpf, p5_area[valid_indices])
        p50_area = np.interp(hpf_centers, valid_hpf, p50_area[valid_indices])
        p95_area = np.interp(hpf_centers, valid_hpf, p95_area[valid_indices])
...
# Fit splines to percentile curves
print("\nFitting UnivariateSpline to percentile curves...")
print(f"  Smoothing parameter (s): {SPLINE_SMOOTHING}")

try:
    spline_p5 = UnivariateSpline(hpf_centers, p5_array, s=SPLINE_SMOOTHING, k=3)
    spline_p50 = UnivariateSpline(hpf_centers, p50_array, s=SPLINE_SMOOTHING, k=3)
    spline_p95 = UnivariateSpline(hpf_centers, p95_array, s=SPLINE_SMOOTHING, k=3)

    # Evaluate splines at original bin centers
    p5_spline = spline_p5(hpf_centers)
    p50_spline = spline_p50(hpf_centers)
    p95_spline = spline_p95(hpf_centers)
    ...
    p5_spline = p5_array
    p50_spline = p50_array
    p95_spline = p95_array
...
output_csv = OUTPUT_DIR / "length_reference_curves_spline.csv"
...
ax = axes[0]
ax.plot(hpf_centers, p50_spline, 'b-', linewidth=2, label='p50 (median)')
ax.fill_between(hpf_centers, p5_spline, p95_spline, alpha=0.3, color='blue', label='p5-p95 range')
ax.plot(hpf_centers, p5_spline, 'b--', linewidth=1, alpha=0.5)
ax.plot(hpf_centers, p95_spline, 'b--', linewidth=1, alpha=0.5)
ax.set_xlabel('Predicted Stage (hpf)', fontsize=12)
ax.set_ylabel('Total Length (µm)', fontsize=12)
ax.set_title('Total Length Reference Curves from Wildtype Controls (Spline Fit)', fontsize=14, fontweight='bold')
ax.legend()
ax.grid(alpha=0.3)
...
output_plot = OUTPUT_DIR / "length_reference_plot_spline.png"
    ...
    for embryo_id in embryo_ids:
        ...
        if len(embryo_data) > 1:
            ax.plot(embryo_data['predicted_stage_hpf'], embryo_data['total_length_um'],
                   alpha=0.1, linewidth=0.5, color='gray', label='Raw embryo trajectories' if embryo_id == embryo_ids[0] else '')

# Plot reference bands
ax.fill_between(hpf_centers, p5_spline, p95_spline, alpha=0.3, color='blue', label='p5-p95 range (reference)')
ax.plot(hpf_centers, p50_spline, 'b-', linewidth=2.5, label='p50 (median)')
ax.plot(hpf_centers, p5_spline, 'b--', linewidth=1.5, alpha=0.7, label='p5')
ax.plot(hpf_centers, p95_spline, 'b--', linewidth=1.5, alpha=0.7, label='p95')

ax.set_xlabel('Predicted Stage (hpf)', fontsize=12)
ax.set_ylabel('Total Length (µm)', fontsize=12)
ax.set_title('Wildtype Total Length Trajectories with Spline-Fitted Reference Bounds', fontsize=14, fontweight='bold')
ax.legend(fontsize=10)
ax.grid(alpha=0.3)

output_traj_plot = OUTPUT_DIR / "length_reference_with_trajectories_spline.png"
...
print(f"\nReference curves (spline-fitted):")
print(f"  p5 range: {np.nanmin(p5_spline):.1f} - {np.nanmax(p5_spline):.1f} µm")
print(f"  p50 range: {np.nanmin(p50_spline):.1f} - {np.nanmax(p50_spline):.1f} µm")
print(f"  p95 range: {np.nanmin(p95_spline):.1f} - {np.nanmax(p95_spline):.1f} µm")
