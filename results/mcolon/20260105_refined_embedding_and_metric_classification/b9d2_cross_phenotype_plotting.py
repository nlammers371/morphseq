#!/usr/bin/env python
"""B9D2 Cross-Phenotype Analysis - Plotting

Visualizes cross-phenotype comparisons:
- HTA vs CE (primary cross-phenotype comparison)
- HTA vs NonPenHets (reference)
- CE vs NonPenHets (reference)
- NonPenHets vs WT (control validation)

Scientific focus: Can we distinguish HTA from CE phenotypes using different
feature modalities? Which features best separate these two penetrant phenotypes?
"""
...
# Color scheme
COLORS = {
    'HTA_vs_CE': '#D32F2F',           # Red (primary cross-phenotype)
    'HTA_vs_NonPenHets': '#FF6F00',   # Orange (HTA reference)
    'CE_vs_NonPenHets': '#1976D2',    # Blue (CE reference)
    'NonPenHets_vs_WT': '#888888',    # Gray (control validation)
}

# Line styles
STYLES = {
    'HTA_vs_CE': '-',                 # Solid (primary)
    'HTA_vs_NonPenHets': '-',         # Solid
    'CE_vs_NonPenHets': '-',          # Solid
    'NonPenHets_vs_WT': '--',         # Dashed (control)
}
...
def create_feature_comparison_figure():
    ...
    Each panel shows curvature, length, or embedding with all 4 comparisons overlaid.
    """
...
def create_individual_comparison_plots():
    ...
    for comp_name, comp_dir in COMPARISONS.items():
        ...
        if 'NonPenHets_vs_WT' in comp_name:
            ...
            title_prefix = 'B9D2 Cross-Phenotype'
...
def create_embedding_overlay():
    ...
    fig = plot_multiple_aurocs(
        ...
        colors_dict=COLORS,
        styles_dict=STYLES,
        title='B9D2 Cross-Phenotype: Embedding-based Classification',
        save_path=str(FIGURES_DIR / 'cross_phenotype_embedding_overlay.png'),
        figsize=(12, 7),
        ylim=(0.3, 1.0)
    )

    print(f"    ✓ Saved: {FIGURES_DIR / 'cross_phenotype_embedding_overlay.png'}")
...
def main():
    """Generate all plots."""
    print("="*70)
    print("B9D2 CROSS-PHENOTYPE PLOTTING")
    print("="*70)
    ...
    print("  - hta_vs_ce.png")
    print("  - hta_vs_nonpenhets.png")
    print("  - ce_vs_nonpenhets.png")
    print("  - nonpenhets_vs_wt.png")
    print("  - cross_phenotype_embedding_overlay.png")
    print("="*70)
