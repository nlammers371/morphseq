"""B9D2 HTA Phenotype Analysis - Data Generation

Runs 9 classifications (3 comparisons × 3 feature types) for HTA phenotype:
1. HTA vs Non-Penetrant Hets (primary comparison)
2. HTA vs WT (secondary comparison)
...
def define_groups(df):
    """Define HTA, Non-Penetrant Het, and WT groups.
    ...
    comparisons = {
        ...
        'HTA_vs_WT': (hta_ids, wt_ids, 'HTA', 'WT'),
...
def print_final_summary(summary_df):
    ...
    """
    print("\n" + "="*70)
    print("FINAL SUMMARY: HTA ANALYSIS")
    print("="*70)
    ...
    hta_wt = summary_df[summary_df['comparison'] == 'HTA_vs_WT']

    if len(hta_nonpen) > 0:
        print("\n2. HTA Detection (HTA vs Non-Pen Hets):")
        for _, row in hta_nonpen.iterrows():
            print(f"   {row['feature']:12s}: AUROC={row['max_auroc']:.3f} "
                  f"(earliest sig: {row['earliest_significant_hpf']} hpf)")

    if len(hta_wt) > 0:
        print("\n3. HTA Detection (HTA vs WT):")
        for _, row in hta_wt.iterrows():
            print(f"   {row['feature']:12s}: AUROC={row['max_auroc']:.3f} "
                  f"(earliest sig: {row['earliest_significant_hpf']} hpf)")
