#!/usr/bin/env python
"""
CEP290 Per-Experiment Analysis - CURVATURE ONLY (Batch Effect Control)

This script runs CEP290 classification using RAW CURVATURE (baseline_deviation_normalized)
instead of VAE embeddings, separately within each experiment to control for batch effects.

Key question: Is curvature alone sufficient to detect phenotypes within each experiment?
...
OUTPUT_DIR = Path(__file__).parent / "per_experiment_results_curvature_only"
...
def run_per_experiment_analysis(df, experiment_id):
    ...
    if cluster_results_valid:
        ...
        fig_cluster = plot_auroc_overlay(
            ...
            colors=COLORS,
            title=f'Per-Cluster Classification vs WT - CURVATURE ONLY (Experiment {experiment_id})',
    ...
    if pooled_results_valid:
        ...
        fig_pooled = plot_auroc_overlay(
            ...
            colors=COLORS,
            title=f'Pooled Classification - CURVATURE ONLY (Experiment {experiment_id})',
            time_bin_width=TIME_BIN_WIDTH
        )
...
def main():
    print("="*70)
    print("CEP290 PER-EXPERIMENT ANALYSIS - CURVATURE ONLY")
    print("="*70)
    print("\n⭐ KEY: Using CURVATURE (baseline_deviation_normalized) instead of VAE embeddings")
