#!/usr/bin/env python
"""
CEP290 Phenotype Statistical Analysis (Updated)

This script performs:
1. Per-cluster classification: Each phenotype cluster (bumpy, high_to_low, low_to_high) vs WT
2. Pooled classification: All Homozygous vs WT, Homo vs Het, Het vs WT
3. All plots include p-value annotations from permutation testing
...
OUTPUT_DIR = Path(__file__).parent / "results_2hr_bins"
...
MIN_SAMPLES_PER_BIN = 3  # Lower threshold for small clusters
...
COLORS = {
    ...
    'Het_vs_WT': '#888888',  # Gray for baseline
    'bumpy': '#9467BD',
    'low_to_high': '#17BECF',
    'high_to_low': '#E377C2',
    'bumpy_vs_WT': '#9467BD',
...
# =============================================================================
# Part 1: Per-Cluster Classification (NEW)
# =============================================================================

def run_cluster_classification(df):
    """
    Run AUROC-based classification for each cluster vs WT.
    Also run Het vs WT as baseline.
    """
    print("\n" + "="*60)
    print("PART 1: Per-Cluster Classification (Each Cluster vs WT)")
    print("="*60)
...
# =============================================================================
# Part 2: Pooled Classification (existing)
# =============================================================================

def run_pooled_classification(df):
    """
    Run AUROC-based classification comparing:
    1. All Homozygous vs WT
    2. All Homozygous vs Het
    3. Het vs WT (baseline)
    """
    print("\n" + "="*60)
    print("PART 2: Pooled Classification Analysis (All Homo vs WT/Het)")
    print("="*60)
...
def plot_cluster_vs_wt_auroc(cluster_results):
    """
    NEW: Plot AUROC for each cluster vs WT with p-value annotations.
    Het vs WT shown as dashed baseline.
    """
    ...
    for cluster in CLUSTERS_TO_ANALYZE:
        ...
        # Add significance markers (larger circles around significant points)
        ...
        # Add stars for highly significant (p < 0.01)
    ...
    ax.set_title('Per-Cluster Classification vs WT\n(VAE Latent Features, 2-hour bins, shaded = null mean ± 1 SD, circles = p<0.05, * = p<0.01)',
                fontsize=14)
...
def plot_pooled_auroc_with_pvalues(all_results):
    """
    UPDATED: Plot AUROC over time for pooled comparisons with p-value annotations.
    """
    fig, ax = plt.subplots(figsize=(14, 7))
    ...
    for label, results in all_results.items():
        ...
        # Add significance markers (circles around significant points)
        sig_mask = df_class['pval'] < 0.05
        if sig_mask.any():
            ax.scatter(df_class.loc[sig_mask, 'time_bin'],
                      df_class.loc[sig_mask, 'auroc_observed'],
...
def plot_temporal_emergence(all_results, title_prefix=""):
    ...
    fig.suptitle(f'{title_prefix}Temporal Emergence of Phenotypic Differences', fontsize=14, y=1.02)
...
def main():
    print("CEP290 Phenotype Statistical Analysis (Updated)")
    print("="*60)
    ...
    # Combine all classification results for CSV export
    ...
    print("\nBaseline (Het vs WT):")
    if 'Het_vs_WT' in cluster_results:
        s = cluster_results['Het_vs_WT']['summary']
        max_auroc = s['max_auroc'] if s['max_auroc'] is not None else 0.0
        print(f"  Max AUROC: {max_auroc:.3f} (should be ~0.5 if Het similar to WT)")

    print("\nOutput files generated:")
    print(f"  - {class_csv_path}")
    print(f"  - {cluster_plot_path}")
    print(f"  - {pooled_plot_path}")
    print(f"  - {temporal_cluster_path}")
    print(f"  - {temporal_pooled_path}")
