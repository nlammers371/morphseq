#!/usr/bin/env python3
"""
Test robust distance methods on CEP290 data.

Same analysis as TMEM67 but for CEP290 to verify methods generalize.
"""
...
def compute_all_distance_methods(df_binned, reference_stats, test_genotype):
    ...
    for idx, row in df_test.iterrows():
        ...
        df_ref_samples = df_binned[
            (df_binned['genotype'].isin(['cep290_wildtype'])) &
...
def plot_method_comparison(df_results, output_path):
    ...
    colors = {'cep290_wildtype': '#2ecc71', 'cep290_heterozygous': '#3498db',
              'cep290_homozygous': '#e74c3c'}
    ...
    plt.suptitle('Comparison of Distance Methods: CEP290 Analysis',
...
def plot_slope_comparison(df_results, output_path):
    ...
    for idx, genotype in enumerate(genotypes):
        ...
        colors_bar = ['red' if p < 0.001 else 'orange' if p < 0.05 else 'gray'
                      for p in df_gen['p_value']]

        ax.barh(df_gen['method'], df_gen['slope'], color=colors_bar)
    ...
    plt.suptitle('Temporal Slope Comparison: CEP290 - Which Methods Show Spurious Trends?',
...
def main():
    print("="*80)
    print("TESTING ROBUST DISTANCE METHODS - CEP290")
    print("="*80)

    # Setup directories
    data_dir = Path(config.DATA_DIR) / "cep290" / "robust_test"
    plot_dir = Path(config.PLOT_DIR) / "cep290" / "robust_test"
    ...
    # ========================================================================
    print("\n" + "="*80)
    print("CEP290 RESULTS SUMMARY")
    print("="*80)

    print("\nMethod performance comparison:")
    for method in ['euclidean', 'mahalanobis_standard', 'mahalanobis_lw',
                   'mahalanobis_oas', 'mahalanobis_pca95']:
        print(f"\n{method}:")

        for genotype in genotypes:
            df_slopes_gen = df_slopes[
                (df_slopes['genotype'] == genotype) &
                (df_slopes['method'] == method)
            ]

            if len(df_slopes_gen) > 0:
                slope = df_slopes_gen.iloc[0]['slope']
                pval = df_slopes_gen.iloc[0]['p_value']
                sig = "***" if pval < 0.001 else "**" if pval < 0.01 else "*" if pval < 0.05 else "ns"
                print(f"  {genotype:25s}: slope={slope:8.4f} (p={pval:.4f}) {sig}")
