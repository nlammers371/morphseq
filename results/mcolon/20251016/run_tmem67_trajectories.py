#!/usr/bin/env python3
"""
TMEM67 Divergence Analysis

Compare tmem67_homozygous and tmem67_heterozygote embryos to tmem67_wildtype reference.
...
def main():
    print("="*80)
    print("TMEM67 DIVERGENCE ANALYSIS")
    print("="*80)

    # Setup directories
    data_dir = Path(config.DATA_DIR) / "tmem67" / "divergence"
    plot_dir = Path(config.PLOT_DIR) / "tmem67" / "divergence"
    ...
    # ========================================================================
    print("\n" + "="*80)
    print("STEP 1: Loading TMEM67 experiments")
    print("="*80)
    ...
    # ========================================================================
    # STEP 3: Compute reference distribution (tmem67_wildtype)
    # ========================================================================
    print("\n" + "="*80)
    print("STEP 3: Computing tmem67_wildtype reference distribution")
    print("="*80)
    ...
    df_hom_div = compute_divergence_scores(
        df_binned,
        ref_stats,
        test_genotypes=['tmem67_homozygous']
    )
    df_hom_div.to_csv(data_dir / "hom_vs_wt_divergence.csv", index=False)
    print(f"  Saved to: {data_dir / 'hom_vs_wt_divergence.csv'}")

    # Heterozygote vs WT (note: tmem67 uses 'heterozygote' not 'heterozygous')
    print("\nComputing heterozygote divergence...")
    df_het_div = compute_divergence_scores(
        df_binned,
        ref_stats,
        test_genotypes=['tmem67_heterozygote']
    )
    ...
    # ========================================================================
    print("\n" + "="*80)
    print("TMEM67 ANALYSIS COMPLETE!")
    print("="*80)
