#!/usr/bin/env python3
"""
Homozygous Ridge Model: Train on Homozygous, Test on Het and WT

Trains Ridge regression models on Homozygous embryos only to learn the
mutant developmental trajectory from 30 hpf.

Then tests on:
1. Homo (held-out) - should predict well (same distribution)
2. Het - check if Het shows any similarity to Homo trajectory
3. WT - baseline comparison (should differ most from Homo model)

This answers: "What is the Homo trajectory, and do Het embryos show any Homo-like patterns?"
"""
...
# Training genotype (Homozygous only)
TRAIN_GENOTYPES = ['cep290_homozygous']
...
OUTPUT_DATA_DIR = DATA_DIR / 'homo_ridge'
OUTPUT_PLOT_DIR = PLOT_DIR / 'homo_ridge'
...
def train_on_genotype_test_on_all(
    ...
    **model_params
    ...
    for genotype_name, test_df in test_dfs.items():
        ...
        # Check if this is the same genotype as training (Homozygous)
        is_training_genotype = (test_df is train_df)

        if is_training_genotype:
            print(f"  Using LOEO for Homo test set (avoid data leakage)")
        ...
        for target_time in sorted(test_pairs['target_time'].unique()):
            ...
            if is_training_genotype:
                # LOEO: For each Homo test embryo, retrain model without that embryo
...
def main():
    print("\n" + "="*80)
    print("HOMOZYGOUS RIDGE MODEL: TRAIN ON HOMOZYGOUS, TEST ON HET AND WT")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    # Get training data (Homozygous only)
    ...
    # ------------------------------------------------------------------------
    # Step 2: Train on Homo, test on all
    # ------------------------------------------------------------------------
    print("\n" + "="*80)
    print("STEP 2: TRAINING ON HOMOZYGOUS, TESTING ON ALL GENOTYPES")
    print("="*80)
    ...
    axes[0].set_title('Homo Ridge Model: Error by Genotype', fontsize=13, fontweight='bold')
    axes[0].legend()
    ...
    axes[1].set_title('Homo Ridge Model: R² by Genotype', fontsize=13, fontweight='bold')
    ...
    for genotype_name in TEST_GENOTYPES:
        ...
        for idx, embryo_id in enumerate(selected_embryos):
            ...
            ax.plot(times, predicted, 's--', color=pred_color, linewidth=2, label='Homo Model Prediction', alpha=0.7)
        ...
        plt.suptitle(f'{genotype_label} Embryos: Actual vs Homo Model Predictions', fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    print("="*80)
    print(f"\nKey Findings:")
    print(f"  - Homo model predicts Homo trajectory (baseline)")
    print(f"  - Compare Het predictions: Do Het show Homo-like patterns?")
    print(f"  - WT predictions: Should differ most (opposite trajectory)")
    print(f"\nOutputs saved to:")
