...
Parallelises over positions within each timepoint using a spawn-context
ProcessPoolExecutor (avoids the fork/nd2 dask handle crash).

Designed to run as a qsub array job, one chunk per T range.

Usage:
  # smoke test — first 2 timepoints, first 5 positions, 4 workers
  conda run -n segmentation_grounded_sam --no-capture-output python \
    results/mcolon/20260423_focus_artifact_detection/10_rel_entropy_full_scan.py \
    --t-limit 2 --p-limit 5 --workers 4

  # full range (for qsub)
  conda run -n segmentation_grounded_sam --no-capture-output python \
    results/mcolon/20260423_focus_artifact_detection/10_rel_entropy_full_scan.py \
    --t-start 0 --t-end 20 --workers 16
...
ND2_PATH   = str(MORPHSEQ_ROOT / "morphseq_playground/raw_image_data/YX1/20250912/20250912_WT_tricane_serial_dilution_experiment.nd2")
MASKS_DIR  = str(MORPHSEQ_ROOT / "morphseq_playground/sam2_pipeline_files/exported_masks/20250912/masks")
...
# ── worker (runs in spawned subprocess — opens its own nd2 handle) ─────────

def _worker(t: int, p: int, well: str) -> dict:
    """Each spawned worker opens the nd2 independently."""
    import numpy as np
    ...
    base = {"t": t, "p": p, "well": well}

    if not mask_path.exists():
        return {**base, "has_mask": False, "rel_entropy_mean": float("nan"),
    ...
    if len(emb_ys) == 0 or len(bg_ys) == 0:
        return {**base, "has_mask": True, "rel_entropy_mean": float("nan"),
                "rel_entropy_min": float("nan"), "rel_entropy_std": float("nan"), "n_z": 0}

    with nd2.ND2File(ND2_PATH) as f:
        stack = f.to_dask()[t, p].compute().astype(np.float32)
    ...
    rels = [_entropy(stack[z][emb_ys, emb_xs]) - _entropy(stack[z][bg_ys, bg_xs])
            for z in range(stack.shape[0])]
    rel = np.array(rels)

    return {**base, "has_mask": True,
...
def main() -> None:
    ...
    with nd2.ND2File(ND2_PATH) as f:
        d = f.to_dask()
        T_total, P_total = d.shape[0], d.shape[1]
    ...
    jobs = [(t, p, p_to_well.get(p, f"p{p:03d}"))
            for t in range(args.t_start, t_end) for p in range(P)]
    total = len(jobs)

    import socket
    print(f"[{socket.gethostname()}] T={args.t_start}–{t_end-1}, P=0–{P-1} → {total} stacks  workers={args.workers}")

    rows: list[dict] = []
    done = 0

    # spawn context: each worker starts a fresh interpreter — no fork/nd2 crash
    ctx = mp.get_context("spawn")
    with ProcessPoolExecutor(max_workers=args.workers, mp_context=ctx) as pool:
        futures = {pool.submit(_worker, t, p, well): (t, p) for t, p, well in jobs}
        for fut in as_completed(futures):
            ...
            try:
                ...
                rows.append({"t": t, "p": p, "well": well, "has_mask": False,
                             "rel_entropy_mean": float("nan"), "rel_entropy_min": float("nan"),
                             "rel_entropy_std": float("nan"), "n_z": 0})
            done += 1
            if done % 50 == 0:
    ...
    print(f"Saved → {out_csv}  ({len(df)} rows, {n_mask} with mask)")
