...
==========================
Sample 2 embryos from each of 10 deciles ranked by focus quality and
produce a ranked Z-stack quality figure using make_ranked_figure().

Ranking metric: rel_entropy_mean (ascending = worst focus first).
Each decile contributes 2 columns → 20 columns total.

Output: figures/decile_ranked_rel_entropy_mean.png
"""
...
SUMMARIES_CSV = HERE / "07_focus_output/focus_summaries.csv"
...
# Ranking metric: sort ascending (low = bad focus), decile 1 = worst
RANK_METRIC   = "rel_entropy_mean"
N_DECILES     = 10
SAMPLES_PER   = 2
DATE          = "20250912"
...
METRICS = [
    ...
    ("rel_entropy_mean", "Rel entropy mean", True),
    ("rel_entropy_min",  "Rel entropy min",  True),
    ("entropy_mean",     "Entropy mean",     True),
    ("entropy_std",      "Entropy std",      False),
]
...
def main() -> None:
    ...
    if RANK_METRIC not in df.columns:
        raise ValueError(
            f"Ranking metric '{RANK_METRIC}' not in CSV. "
    ...
    for ex in examples:
        print(
            f"  {ex['label']:6s}  well={ex['well']:3s}  t={ex['t']:3d}  "
