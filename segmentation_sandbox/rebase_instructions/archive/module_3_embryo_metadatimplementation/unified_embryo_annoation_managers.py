...
from embryo_metadata_models import Phenotype, Flag, Genotype, Treatment
from parsing_utils import FrameAwareRangeParser, extract_frame_number, parse_entity_id
...
class EmbryoFlagManager:
    ...
    def _detect_level(self, entity_id: str) -> str:
        ...
        """
        # Pseudocode: Parse ID to determine if snip/video/image/experiment/embryo
        if self.validate_id_format(entity_id, "snip"):
            return "snip"
        elif self.validate_id_format(entity_id, "video"):
            return "video"
        elif self.validate_id_format(entity_id, "image"):
            return "image"
        elif self.validate_id_format(entity_id, "experiment"):
            return "experiment"
        else:
            return "embryo"  # fallback
...
class UnifiedEmbryoManager(EmbryoPhenotypeManager, EmbryoGenotypeManager, 
    ...
    def _check_dead_conflicts(self) -> Dict:
        ...
        for embryo_id, embryo_data in self.data["embryos"].items():
            ...
            for snip_id, snip_data in embryo_data["snips"].items():
                phenotype_values = [p["value"] for p in snip_data.get("phenotypes", [])]
                
                # Check for DEAD coexistence in same snip
                if "DEAD" in phenotype_values and len(phenotype_values) > 1:
                    conflicts.append({
                        "embryo_id": embryo_id,
                        "snip_id": snip_id,
                        "conflicting_phenotypes": [p for p in phenotype_values if p != "DEAD"]
                    })
    
            
            # Check temporal DEAD logic - no return from DEAD
            if "DEAD" in phenotype_values:
                # Get snips with frame numbers for temporal ordering
                # Refer to id_parsing_conventions.md for snip_id frame format
                snip_frames = []
    ...
    def get_embryo_summary(self, embryo_id: str) -> Dict:
        ...
        # Find earliest DEAD snip (refer to id_parsing_conventions.md for frame format)
        death_snip_id = None
        if phenotypes:
        ...
        return {
            "genotype": self.get_genotype(embryo_id),
            "phenotype_count": len(phenotypes),
            "treatment_count": len(self.get_treatments(embryo_id)),
            "flag_count": len(self.get_flags(embryo_id, "embryo")),
            "snip_count": len(self.get_available_snips(embryo_id)),
    ...
    def copy_embryo_structure(self, source_id: str, target_id: str):
        ...
        def remove_phenotype(self, snip_id: str, phenotype: str = None) -> bool:
            """Remove specific phenotype or all phenotypes from snip."""
            embryo_id = self.get_embryo_id_from_snip(snip_id)
            if not embryo_id:
                return False
            
            snip_phenotypes = self.data["embryos"][embryo_id]["snips"][snip_id]["phenotypes"]
            
            if phenotype:
                # Remove specific phenotype
                original_len = len(snip_phenotypes)
                snip_phenotypes[:] = [p for p in snip_phenotypes if p["value"] != phenotype]
                removed = len(snip_phenotypes) < original_len
            else:
                # Remove all phenotypes
                removed = bool(snip_phenotypes)
                snip_phenotypes.clear()
        ...
        def remove_genotype(self, embryo_id: str) -> bool:
            """Remove genotype from embryo."""
            if embryo_id in self.data["embryos"] and self.data["embryos"][embryo_id]["genotype"]:
                self.data["embryos"][embryo_id]["genotype"] = None
                self._update_timestamps(embryo_id)
                return True
            return False

    def edit_phenotype(self, snip_id: str, phenotype: str, 
                    confidence: float = None, notes: str = None) -> bool:
        """Edit specific phenotype properties."""
        embryo_id = self.get_embryo_id_from_snip(snip_id)
        if not embryo_id:
            return False
        
        phenotypes = self.data["embryos"][embryo_id]["snips"][snip_id]["phenotypes"]
        
        for p in phenotypes:
            if p["value"] == phenotype:
                if confidence is not None:
                    p["confidence"] = confidence
                if notes is not None:
                    p["notes"] = notes
                p["last_updated"] = self.get_timestamp()
                self._update_timestamps(embryo_id)
                return True
        ...
        def edit_genotype(self, embryo_id: str, allele: str = None, 
                        ...
                        notes: str = None) -> bool:
            """Edit genotype properties."""
            if embryo_id not in self.data["embryos"] or not self.data["embryos"][embryo_id]["genotype"]:
                return False
            
            genotype = self.data["embryos"][embryo_id]["genotype"]
            
            if allele is not None:
                genotype["allele"] = allele
            if zygosity is not None:
                if zygosity not in self._get_valid_zygosity_types():
                    raise ValueError(f"Invalid zygosity '{zygosity}'")
                genotype["zygosity"] = zygosity
            if confidence is not None:
                genotype["confidence"] = confidence
            if notes is not None:
                genotype["notes"] = notes
            
            genotype["last_updated"] = self.get_timestamp()
            self._update_timestamps(embryo_id)
            return True

        def remove_flag(self, entity_id: str, flag_type: str, level: str = "auto") -> bool:
            """Remove specific flag from entity."""
            if level == "auto":
                level = self._detect_level(entity_id)
            ...
            if level == "snip":
                ...
                if not embryo_id:
                    return False
                flags = self.data["embryos"][embryo_id]["snips"][entity_id]["flags"]
                original_len = len(flags)
                flags[:] = [f for f in flags if f.get("flag_type") != flag_type]
                return len(flags) < original_len
            
            # Add other levels as needed
            return False

        def remove_treatment(self, embryo_id: str, treatment_id: str) -> bool:
            """Remove specific treatment from embryo."""
            if embryo_id in self.data["embryos"]:
                treatments = self.data["embryos"][embryo_id]["treatments"]
        ...
        def edit_flag(self, entity_id: str, flag_type: str, 
                    priority: str = None, notes: str = None, level: str = "auto") -> bool:
                ...
                for flag in flags:
                    if flag.get("flag_type") == flag_type:
                        if priority is not None:
                            if priority not in self._get_valid_priority_levels():
                                raise ValueError(f"Invalid priority '{priority}'")
                            flag["priority"] = priority
                        if notes is not None:
                            flag["notes"] = notes
                        flag["last_updated"] = self.get_timestamp()
                        self._update_timestamps(embryo_id)
                        return True
            
            return False

        def edit_treatment(self, embryo_id: str, treatment_id: str,
                        dosage: str = None, timing: str = None, notes: str = None) -> bool:
            """Edit treatment properties."""
            if embryo_id in self.data["embryos"] and treatment_id in self.data["embryos"][embryo_id]["treatments"]:
                treatment = self.data["embryos"][embryo_id]["treatments"][treatment_id]
                ...
                treatment["last_updated"] = self.get_timestamp()
                self._update_timestamps(embryo_id)
                return True
        ...
        def get_phenotype_statistics(self) -> Dict[str, int]:
            """Get phenotype distribution statistics."""
            stats = {}
            for embryo_data in self.data["embryos"].values():
                for snip_data in embryo_data["snips"].values():
