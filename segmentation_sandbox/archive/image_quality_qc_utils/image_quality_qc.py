...
Command-line script for image quality control flags in the MorphSeq pipeline.
Supports both manual and automatic QC operations including flagging, removing, and checking QC annotations for images.

Usage Examples:
    # Flag specific images as FAIL
    python utils/image_quality_qc_utils/image_quality_qc.py flag \
    ...
    python utils/image_quality_qc_utils/image_quality_qc.py remove \
        --data_dir /path/to/data \
        --image_ids 20241215_A01_t001 \
        --annotator nlammers

    # Check QC status of images
    python utils/image_quality_qc_utils/image_quality_qc.py check \
        --data_dir /path/to/data \
        --image_ids 20241215_A01_t001,20241215_A01_t002

    # Initialize QC file from experiment metadata
    python utils/image_quality_qc_utils/image_quality_qc.py init \
        --data_dir /path/to/data \
        --overwrite
"""
...
from image_qc_utils import (
    ...
    QC_FLAGS, load_qc_data, generate_comprehensive_qc, initialize_qc_file
)
...
def initialize_qc(args):
    ...
    try:
        ...
        print(f"Successfully initialized QC file with {len(qc_df)} images")
...
def main():
    parser = argparse.ArgumentParser(
        description="Image quality control for MorphSeq pipeline - supports both manual and automatic QC operations",
    ...
    init_parser = subparsers.add_parser(
        'init', parents=[parent_parser], 
        help='Initialize QC file from experiment metadata'
    )
    init_parser.add_argument(
        "--overwrite", action="store_true",
        help="Overwrite existing QC file if it exists"
    )
