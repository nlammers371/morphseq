...
DEFAULT_QC_FLAG_CATEGORIES = {
    ...
    "video_level": {
>>>>>>> fed4db2 (refactored to consoldiate qc jsons into one file and to maintain this)
    },
    "image_level": {
        "BLUR": "Image is blurry (low variance of Laplacian)",
        "DRY_WELL": "Well dried out during imaging",
...
def gen_flag_batch(level: str, entity_id: str, qc_flag: Union[str, List[str]], author: str, 
    ...
    Args:
        ...
        qc_flag: Single QC flag or list of QC flags
        author: Who added the flag(s)
        notes: Optional additional details (applied to all flags if multiple)
        qc_data: QC data dictionary (required if apply_directly=True)
        apply_directly: If True, apply to qc_data immediately. If False, return batch.
    
    Returns:
        List of flag batch entries (if apply_directly=False) or None (if apply_directly=True)
    
    Examples:
        # Mode 1: Return batches for accumulation (DEFAULT)
        batch = []
        batch += gen_flag_batch("image", "img_001", "BLUR", "auto", "Score: 25")
        batch += gen_flag_batch("image", "img_002", ["BLUR", "DRY_WELL"], "analyst", "Multiple issues")
        
        # Apply accumulated batch later
        qc_data = load_qc_data(quality_control_dir)
        qc_data = add_flags_to_qc_data(qc_data, batch)
        save_qc_data(qc_data, quality_control_dir)
        
        # Mode 2: Apply directly to QC data (apply_directly=True)
        qc_data = load_qc_data(quality_control_dir)
        gen_flag_batch("experiment", "20241215", "PROTOCOL_DEVIATION", "tech", "Missing controls", 
                      qc_data=qc_data, apply_directly=True)
        gen_flag_batch("video", "20241215_A01", "TECHNICAL_ISSUE", "operator", "Focus drift",
                      qc_data=qc_data, apply_directly=True)
        gen_flag_batch("image", "20241215_A01_t001", ["BLUR", "OVEREXPOSURE"], "auto", "Multiple issues",
                      qc_data=qc_data, apply_directly=True)
        save_qc_data(qc_data, quality_control_dir)  # Save when ready
        
        # Conditional application with direct mode
        qc_data = load_qc_data(quality_control_dir)
        for image_id, analysis_result in image_analysis.items():
            if analysis_result['blur_score'] < blur_threshold:
                gen_flag_batch("image", image_id, "BLUR", "auto", f"Score: {analysis_result['blur_score']}", 
                              qc_data=qc_data, apply_directly=True)
            if analysis_result['is_dry']:
                gen_flag_batch("image", image_id, "DRY_WELL", "auto", "Automated detection",
                              qc_data=qc_data, apply_directly=True)
        save_qc_data(qc_data, quality_control_dir)
    """
    # Handle both single flag and list of flags
    if isinstance(qc_flag, str):
        qc_flags = [qc_flag]
    else:
        qc_flags = qc_flag
    
    # Generate batch entries for all flags
    batch_entries = []
    ...
    if apply_directly:
        if qc_data is None:
            raise ValueError("qc_data must be provided when apply_directly=True")
        
        # Apply flags directly to QC data
        modified_qc_data = add_flags_to_qc_data(qc_data, batch_entries, validate_flags=True)
        
        # Update the original qc_data dictionary in place
        qc_data.clear()
        qc_data.update(modified_qc_data)
...
def add_flags_to_qc_data(qc_data: Dict, flag_batch: List[Dict], validate_flags: bool = True) -> Dict:
    """
    Apply a batch of flags to QC data in memory.
    
    This function applies all flags in a batch to the QC data object
    without any file I/O. It validates flags against JSON-defined
    categories to ensure only valid flags are applied.
    ...
    Example:
        ...
        # Build batch
        batch = []
        batch += gen_flag_batch("image", "img_001", "BLUR", "auto")
        batch += gen_flag_batch("image", "img_002", "DRY_WELL", "manual")
        ...
        save_qc_data(qc_data, quality_control_dir)
    """
    for flag_entry in flag_batch:
        # Validate required fields
        required_fields = ["level", "entity_id", "qc_flag", "author"]
        for field in required_fields:
            if field not in flag_entry:
                raise ValueError(f"Missing required field '{field}' in flag entry: {flag_entry}")
        
        level = flag_entry["level"]
        entity_id = flag_entry["entity_id"]
        qc_flag = flag_entry["qc_flag"]
        author = flag_entry["author"]
        notes = flag_entry.get("notes", "")
        
        # Validate flag if requested
        if validate_flags:
            if not is_valid_qc_flag(qc_data, level, qc_flag):
                valid_flags = qc_data.get("valid_qc_flag_categories", {}).get(f"{level}_level", {})
                raise ValueError(
                    f"Invalid QC flag '{qc_flag}' for level '{level}'. "
                    f"Valid flags: {list(valid_flags.keys())}"
                )
        
        # Determine parent_ids based on level and entity_id
        parent_ids = None
        if level == "image":
            parent_ids = parse_image_id(entity_id)
        elif level == "embryo":
        ...
        # Apply the flag directly to the in-memory QC data
        qc_data = _add_qc_flag_to_data(qc_data, level, entity_id, qc_flag, author, notes, parent_ids)
...
def get_qc_summary(quality_control_dir: Union[str, Path]) -> Dict:
    ...
    Example:
        summary = get_qc_summary("/path/to/quality_control")
        print(f"Total BLUR flags: {summary['image_level'].get('BLUR', 0)}")
        print(f"Total experiments with issues: {len(summary['experiment_level'])}")
    """
    qc_data = load_qc_data(quality_control_dir)
...
def _add_qc_flag_to_data(qc_data: Dict, level: str, entity_id: str, qc_flag: str, 
                        author: str, notes: str, parent_ids: Dict = None) -> Dict:
    """Internal function to add a single flag directly to QC data structure."""
    # Ensure experiments section exists
    if "experiments" not in qc_data:
        qc_data["experiments"] = {}
    ...
    if level == "experiment":
        ...
        # Add the flag
        qc_data["experiments"][exp_id]["flags"].append(qc_flag)
        qc_data["experiments"][exp_id]["authors"].append(author)
        qc_data["experiments"][exp_id]["notes"].append(notes)
    
    elif level == "video" and parent_ids:
        exp_id = parent_ids["experiment_id"]
        if exp_id not in qc_data["experiments"]:
            qc_data["experiments"][exp_id] = {"flags": [], "authors": [], "notes": [], "videos": {}}
        if "videos" not in qc_data["experiments"][exp_id]:
            qc_data["experiments"][exp_id]["videos"] = {}
        if entity_id not in qc_data["experiments"][exp_id]["videos"]:
            qc_data["experiments"][exp_id]["videos"][entity_id] = {"flags": [], "authors": [], "notes": [], "images": {}, "embryos": {}}
        
        # Add the flag
        qc_data["experiments"][exp_id]["videos"][entity_id]["flags"].append(qc_flag)
        qc_data["experiments"][exp_id]["videos"][entity_id]["authors"].append(author)
        qc_data["experiments"][exp_id]["videos"][entity_id]["notes"].append(notes)
    
    elif level == "image" and parent_ids:
        exp_id = parent_ids["experiment_id"]
        ...
        if "videos" not in qc_data["experiments"][exp_id]:
            qc_data["experiments"][exp_id]["videos"] = {}
        if video_id not in qc_data["experiments"][exp_id]["videos"]:
            qc_data["experiments"][exp_id]["videos"][video_id] = {"flags": [], "authors": [], "notes": [], "images": {}, "embryos": {}}
        if "images" not in qc_data["experiments"][exp_id]["videos"][video_id]:
            qc_data["experiments"][exp_id]["videos"][video_id]["images"] = {}
        if entity_id not in qc_data["experiments"][exp_id]["videos"][video_id]["images"]:
            qc_data["experiments"][exp_id]["videos"][video_id]["images"][entity_id] = {
                "flags": [], "authors": [], "notes": []
            }
        
        # Add the flag
        qc_data["experiments"][exp_id]["videos"][video_id]["images"][entity_id]["flags"].append(qc_flag)
        qc_data["experiments"][exp_id]["videos"][video_id]["images"][entity_id]["authors"].append(author)
        qc_data["experiments"][exp_id]["videos"][video_id]["images"][entity_id]["notes"].append(notes)
    
    elif level == "embryo" and parent_ids:
        exp_id = parent_ids["experiment_id"]
        video_id = parent_ids["video_id"]
        ...
        if video_id not in qc_data["experiments"][exp_id]["videos"]:
            qc_data["experiments"][exp_id]["videos"][video_id] = {"flags": [], "authors": [], "notes": [], "images": {}, "embryos": {}}
        if "embryos" not in qc_data["experiments"][exp_id]["videos"][video_id]:
            qc_data["experiments"][exp_id]["videos"][video_id]["embryos"] = {}
        if entity_id not in qc_data["experiments"][exp_id]["videos"][video_id]["embryos"]:
            qc_data["experiments"][exp_id]["videos"][video_id]["embryos"][entity_id] = {
                "flags": [], "authors": [], "notes": []
            }
        
        # Add the flag
        target = qc_data["experiments"][exp_id]["videos"][video_id]["embryos"][entity_id]
        target["flags"].append(qc_flag)
        target["authors"].append(author)
        target["notes"].append(notes)
    
    return qc_data

def is_valid_qc_flag(qc_data: Dict, level: str, qc_flag: str) -> bool:
    """Helper function to validate QC flags."""
    valid_flags = qc_data.get("valid_qc_flag_categories", {}).get(f"{level}_level", {})
    return qc_flag in valid_flags
