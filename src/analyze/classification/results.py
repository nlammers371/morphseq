...
class MulticlassOVRResults:
    ...
    embryo_predictions: Optional[pd.DataFrame] = None
    null_summary: Optional[pd.DataFrame] = None
    confusion_profile: Optional[pd.DataFrame] = None
    ...
    def save(
        ...
        # Save optional stage-1 artifacts used by Stage 2
        if self.embryo_predictions is not None:
            emb_path = path / 'embryo_predictions_augmented.parquet'
            if emb_path.exists() and not overwrite:
                raise FileExistsError(
                    f"{emb_path} exists. Use overwrite=True to replace."
                )
            self.embryo_predictions.to_parquet(emb_path, index=False)

        if self.confusion_profile is not None:
            conf_path = path / 'confusion_profile.parquet'
            if conf_path.exists() and not overwrite:
                raise FileExistsError(
                    f"{conf_path} exists. Use overwrite=True to replace."
                )
            self.confusion_profile.to_parquet(conf_path, index=False)

        if self.null_summary is not None:
            null_dir = path / 'null'
            null_dir.mkdir(parents=True, exist_ok=True)
            null_path = null_dir / 'auroc_null_summary.parquet'
            if null_path.exists() and not overwrite:
                raise FileExistsError(
                    f"{null_path} exists. Use overwrite=True to replace."
                )
            self.null_summary.to_parquet(null_path, index=False)

            stage1_meta = self.metadata.get('stage1_null_metadata', {})
            null_meta_path = null_dir / 'null_metadata.json'
            if null_meta_path.exists() and not overwrite:
                raise FileExistsError(
                    f"{null_meta_path} exists. Use overwrite=True to replace."
                )
            with open(null_meta_path, 'w') as f:
                json.dump(self._prepare_metadata_for_json(stage1_meta), f, indent=2, default=str)

            if (
                self.metadata.get('null_save_mode') == 'full'
                and {'class', 'time_bin', 'null_mean', 'null_std'}.issubset(self.null_summary.columns)
            ):
                full_path = null_dir / 'auroc_null_full.npz'
                if full_path.exists() and not overwrite:
                    raise FileExistsError(
                        f"{full_path} exists. Use overwrite=True to replace."
                    )
                # Persist summary arrays as a lightweight fallback for "full" mode.
                np.savez_compressed(
                    full_path,
                    cls=self.null_summary['class'].to_numpy(),
                    time_bin=self.null_summary['time_bin'].to_numpy(),
                    null_mean=self.null_summary['null_mean'].to_numpy(),
                    null_std=self.null_summary['null_std'].to_numpy(),
                )
    ...
    def from_dir(cls, path: Union[str, Path]) -> "MulticlassOVRResults":
        ...
        emb_path = path / 'embryo_predictions_augmented.parquet'
        embryo_predictions = pd.read_parquet(emb_path) if emb_path.exists() else None

        conf_path = path / 'confusion_profile.parquet'
        confusion_profile = pd.read_parquet(conf_path) if conf_path.exists() else None

        null_path = path / 'null' / 'auroc_null_summary.parquet'
