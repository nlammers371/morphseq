...
## ClassificationAnalysis object, layers, persistence, uns — see OUTPUT_SPEC

The following sections have moved to [`OUTPUT_SPEC.md`](OUTPUT_SPEC.md), the
user-facing output contract:

- `ClassificationAnalysis` object (fields, methods, subset/stack semantics)
- `_LazyLayers` — lazy artifact registry
- `NullDistributions` — raw null arrays
- `_validate_scores` rules
- On-disk layout
- Artifact tiers (what's on by default, what requires a flag)
- Misclassification pipeline fail-loud contract
- `uns` structure

DESIGN.md retains only the **rationale** for why these shapes were chosen.
The authoritative contract lives in OUTPUT_SPEC.md.
