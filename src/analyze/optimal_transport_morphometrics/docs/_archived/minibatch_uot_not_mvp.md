# Task 01: Minibatch Unbalanced Optimal Transport

## Overview
Implement minibatch UOT to produce teacher velocities for the neural ODE, with support for trajectory-aware biasing.

## Mathematical Foundation

### Core UOT Problem
Given source points **X** = {x_i}_{i=1}^m at time t and targets **Y** = {y_j}_{j=1}^n at time t+Δt:

```
min_Π  ⟨Π, C⟩ + ε·KL(Π || a⊗b) + τ[KL(Π𝟙 || a) + KL(Π^T𝟙 || b)]
```

Where:
- **C_ij** = ||x_i - y_j||² (cost matrix)
- **ε** = entropic regularization (smoothness)
- **τ** = mass penalty (unbalanced regularization)
- **a, b** = source/target mass distributions (usually uniform)

### Teacher Velocity Extraction
From transport plan Π:
1. Row mass: **r_i** = Σ_j Π_ij
2. Teacher velocity: **v̂_i** = (Σ_j Π_ij(y_j - x_i)) / (Δt · r_i) if r_i > 0

## Implementation Guide

### Step 1: Basic UOT Function
...
### Step 2: Trajectory-Biased UOT (Three Methods)
...
### Step 3: Minibatch Sampling Strategy
...
1. **Entropic regularization (ε)**
   - Range: [0.01, 0.5]
   - Small ε → sharp transport (but numerically unstable)
   - Large ε → blurry transport (but stable)
   - **Start with**: 0.05

2. **Mass penalty (τ = reg_m)**
   - Range: [0.1, 10.0]
   - Small τ → easy mass creation/deletion
   - Large τ → preserve total mass
   - **Start with**: 1.0

3. **Bias strength**
   - Cost shaping (γ): [1.0, 10.0]
   - Prior regularization (λ): [0.1, 2.0]
   - Lower bound fraction: [0.3, 0.8]
   - **Start with**: moderate values, tune based on tracking quality
...
### Problem 1: All mass gets deleted
**Symptom**: row_masses ≈ 0 everywhere  
**Solution**: Increase reg_m (mass penalty)

### Problem 2: Transport is too blurry
**Symptom**: Every source connects to every target  
**Solution**: Decrease ε (entropic regularization)

### Problem 3: Self-edges ignored despite tracking
**Symptom**: Known tracks not preserved  
**Solution**: Increase bias_strength or use lower_bound mode

### Problem 4: Numerical instability
**Symptom**: NaN values in transport plan  
**Solution**: Increase ε, check for extreme cost values

### Problem 5: Empty target clusters
**Symptom**: No points in target time window  
**Solution**: Increase time_tolerance or handle gracefully with pure deletion
...
## Integration with Spectral Clustering (Task 02)

The minibatch sampler expects cluster labels from spectral clustering:
...
1. First implement and test basic `compute_minibatch_uot`
2. Add one biasing mode at a time, test each
3. Integrate with spectral clustering (Task 02)
4. Add calibrated lower bounds (Task 03)
5. Wire into ODE training loop
6. Profile and optimize if needed

Remember: Start simple, test often, visualize transport plans!