...
from .core import BranchTestResult
...
def plot_tree_schematic(
    ...
    x-axis: condensed x or y (depending on spatial_axis)
    y-axis: time (hpf, back-converted from t_scaled)
    ...
    for _, e in edges_df.iterrows():
        ...
        xa, xb = float(nodes_df.loc[a, sp_col]), float(nodes_df.loc[b, sp_col])
        ya, yb = float(nodes_hpf.iloc[a]), float(nodes_hpf.iloc[b])
        ax.plot([xa, xb], [ya, yb],
    ...
    for _, row in nodes_df.iterrows():
        if row["degree"] >= 3:
            continue
        ax.scatter(
            row[sp_col], nodes_hpf.iloc[int(row["node_id"])],
    ...
    for bn, res in br_map.items():
        bx = float(nodes_df.loc[bn, sp_col])
        by = float(nodes_hpf.iloc[bn])
    ...
    ax.set_xlabel(sp_label, fontsize=10)
