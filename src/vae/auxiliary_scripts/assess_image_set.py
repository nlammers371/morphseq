...
def assess_image_set(image_path, metadata_path, trained_model_path, out_path, image_prefix_list="", rs_factor=1.0, batch_size=64):
    ...
    for i in tqdm(range(len(image_path_list))):
        ...
        if i == 0:
            ...
            rs_dims = img.shape
            image_tensor = torch.zeros((len(image_snip_names), 1, rs_dims[0], rs_dims[1]))

        image_tensor[i, 0, :, :] = img
    ...
    trained_model = set_inputs_to_device(trained_model, device)
    # get the dataloader
    ...
    for n in tqdm(range(n_batches)):
        ...
        x = set_inputs_to_device(inputs, device)
