...
def compute_stage_predictions_well(
    *,
    ...
    if fm.empty:
        raise ValueError(f"No frame_manifest rows for experiment={exp} well_id={well}.")
    ...
    # Join seg -> frame_manifest to get elapsed_time_s.
    ...
    if merged["elapsed_time_s"].isna().any():
        ...
        raise ValueError(f"Missing elapsed_time_s after join to frame_manifest (preview): {preview}")
