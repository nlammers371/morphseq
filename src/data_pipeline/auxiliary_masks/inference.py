...
def _predict_family_masks(
    *,
    ...
    path_col = "source_image_path" if "source_image_path" in frame_df.columns else "stitched_image_path"
    source_paths = [str(p) for p in frame_df[path_col].tolist()]
    image_lookup = {str(row[path_col]): str(row["image_id"]) for _, row in frame_df.iterrows()}
