...
def apply_unet(root, model_name, n_classes, overwrite_flag=False, segment_list=None, im_dims=None, batch_size=64,
    ...
    image_path_list = glob.glob(os.path.join(image_root, '*.png')) + glob.glob(os.path.join(image_root, '*.tif')) + glob.glob(os.path.join(image_root, '*.jpg'))
    ...
    label_path_list = []
    exist_flags = []
    label_path_root = os.path.join(root, "built_image_data", "segmentation")
    for imp in image_path_list:
    ...
    with (torch.no_grad()):
        ...
        for idx in tqdm(range(len(im_dataloader)), "Segmenting images..."):
            ...
            for b in range(lb_predicted.shape[0]):
                ...
                im_path = im_paths[b]
                suffix = im_path.replace(image_root, "")
