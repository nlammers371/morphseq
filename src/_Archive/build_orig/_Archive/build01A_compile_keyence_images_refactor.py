...
def process_well(w, well_list, cytometer_flag, ff_dir, depth_dir, ch_to_use=1, overwrite_flag=False, no_timelapse_flag=False):
    ...
    for t, time_dir in enumerate(time_dir_list):
        ...
        temp_dict = scrape_keyence_metadata(im_list[0])
...
def build_ff_from_keyence(data_root, overwrite_flag=False, ch_to_use=1, dir_list=None, write_dir=None, no_timelapse_flag=False):
    ...
    for d in dir_indices:
        ...
        stitch_dir = os.path.join(write_dir, "stitched_images",  sub_name)
...
def process_keyence_images(root, overwrite_flag, dir_list=None, write_dir=None, ch_to_use=None):

    read_dir = os.path.join(root, 'raw_image_data', 'keyence', '') 
    if write_dir is None:
        write_dir = os.path.join(root, 'built_image_data', 'keyence', '')
    ...
    ### step 1: loop through all experiment directories and build map of data paths and types
    for d in dir_indices:
        ...
        print(f'Building metadata for images in directory {d+1:01} of ' + f'{len(dir_indices)}')
        ...
        if len(metadata_df) > 0:
            ...
            metadata_df.to_csv(metadata_path, index_col=False)

    
    ### step 2: stitch Z slices
