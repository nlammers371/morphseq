...
def calculate_UMAPs(embryo_df, trained_model, n_components=3):
    ...
    z_mu_array = embryo_df.loc[:, mu_indices].to_numpy()
    ...
    if trained_model.model_name == "MetricVAE":
        ...
        scaled_z_mu_bio = StandardScaler().fit_transform(z_mu_array[:, zmb_indices])
        ...
        scaled_z_mu_n = StandardScaler().fit_transform(z_mu_array[:, zmn_indices])
...
def initialize_assessment(train_dir, output_dir, mode_vec=None):
    ...
    meta_df = pd.read_csv(os.path.join(figure_path, "meta_summary_df"))
    vae_df = pd.read_csv(os.path.join(figure_path, "embryo_stats_df"))

    return trained_model, vae_df, meta_df, figure_path, data_sampler_vec, continue_flag
...
if __name__ == "__main__":
    ...
    for m_iter, model_name in enumerate(models_to_assess):
        ...
        trained_model, vae_df, meta_df, figure_path, data_sampler_vec, continue_flag = initialize_assessment(train_dir, output_dir)
        if continue_flag:
            continue
        ...
        umap_df = calculate_UMAPs(vae_df, trained_model)
        print(f"Saving data...")
